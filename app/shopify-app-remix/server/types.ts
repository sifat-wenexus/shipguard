...
import { deriveApi } from '~/shopify-app-remix/server/shopify-app';
...
interface Authenticate<Config extends AppConfigArg> {
   ...
   * import { LATEST_API_VERSION, shopifyApp } from "~/shopify-app-remix/server";
   ...
   * ```ts
   * // /app/shopify.server.ts
   * import { DeliveryMethod, shopifyApp } from "~/shopify-app-remix/server";
   *
   * const shopify = shopifyApp({
   *   webhooks: {
   *    APP_UNINSTALLED: {
...
export interface ShopifyAppBase<Config extends AppConfigArg> {
   ...
   * <description>Import the `~/shopify-session-storage-prisma` package to store sessions in your Prisma database.</description>
   * ```ts
   * // /app/shopify.server.ts
  ...
  api: ReturnType<typeof deriveApi>;
}
